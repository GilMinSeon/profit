			...
			<div class="classes__item__text" style="text-align: right;padding-top: 0;">
            	...
            	<div class="col-lg-12">
					...
					<c:if test="${pageMaker.prev}">
						<a href="trainerPermitList.do${pageMaker.makeQueryChatList(pageMaker.startPage - 1)}">
					...
					<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
            			<a href="chatAccountInfo.do${pageMaker.makeQueryChatList(idx)}" <c:if test="${page == idx }">style="background: #5768AD;color:#FFFFFF;"</c:if>>${idx}</a>
					</c:forEach>
					
					<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
						<a href="chatAccountInfo.do${pageMaker.makeQueryChatList(pageMaker.endPage + 1)}"><span class="arrow_carrot-right"></span></a>
					</c:if>
					</div>
				</div>
			</div>
